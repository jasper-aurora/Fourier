...
if(strcmp(Theory,"Stokes")==0)
	{
	if(n<=5) // sprintf(Method, "\n# Solution by %d-order Stokes theory", n); // JP
		snprintf(Method, 100, "\n# Solution by %d-order Stokes theory", n); // JP
	else
		{
		n = 5;
...
if(strcmp(Theory,"Fourier")==0)
	// sprintf(Method, "\n# Solution by %d-term Fourier series", n); // JP
	snprintf(Method, 100, "\n# Solution by %d-term Fourier series", n); // JP
...
fprintf(file,"# %s", Title);
fprintf(file,"\n%s\n", Method);
fprintf(file,"\n# Height/Length:%6.3f, %3.0lf\%% of the maximum of H/L = %.16f",
...
int 		i, I;
double 	X, eta, y;
double	Surface(double) ; // , Point(double, double); // JP
void Point(double, double); // JP
